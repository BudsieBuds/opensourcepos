...
<form id="copy-issue">
    ...
    if (!((substr(decoct(fileperms($logs)), -4) == 750)
        ...
        {
            echo '<li class="card-text">' . $bullet . '<code>writable/uploads</code> ' . lang('Config.is_writable') . '</li>';
        }

        if (substr(decoct(fileperms($images)), -4) != 750)
        {
            echo '<li class="card-text">' . $bullet . '<code>writable/uploads/item_pics</code> ' . lang('Config.is_writable') . '</li>';
        }
