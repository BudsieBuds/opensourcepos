	...
	<?= form_open("$controller_name/changeMode", ['id' => 'mode_form', 'class' => 'form-horizontal panel panel-default']) ?>
		<div class="panel-body form-group">
			<ul>
				...
				?>
					<li class="pull-right">
						<?= anchor("$controller_name/manage", '<i class="bi-receipt-cutoff icon-spacing"></i>' . lang(ucfirst($controller_name) .'.takings'),
			...
			?>
					<?= form_open("$controller_name/editItem/$line", ['class' => 'form-horizontal', 'id' => "cart_$line"]) ?>
						<tr>
							...
							<td><a href="javascript:document.getElementById('<?= "cart_$line" ?>').submit();" title=<?= lang(ucfirst($controller_name) .'.update') ?> ><i class="bi-arrow-repeat"></i></a></td>
						</tr>
						<tr>
							<?php
							if($item['item_type'] == ITEM_TEMP)
		...
		?>
			...
			<div id="payment_details">
				...
				?>
					...
					if($pos_mode)
					{
						...
						{
					?>
							<div class='btn btn-sm btn-success pull-right' id='finish_sale_button' tabindex="<?= ++$tabindex ?>"><i class="bi-check-circle icon-spacing"></i><?= lang(ucfirst($controller_name) .'.complete_sale') ?></div>
					<?php
						}
					}
					?>
				...
				?>
					...
					<div class='btn btn-sm btn-success pull-right' id='add_payment_button' tabindex="<?= ++$tabindex ?>"><i class="bi-cash-coin icon-spacing"></i><?= lang(ucfirst($controller_name) .'.add_payment') ?></div>
				<?php
				}
				?>
			...
			<?= form_open("$controller_name/cancel", ['id' => 'buttons_form']) ?>
				<div class="form-group" id="buttons_sale">
					<div class='btn btn-sm btn-default pull-left' id='suspend_sale_button'><i class="bi-pause-circle icon-spacing"></i><?= lang(ucfirst($controller_name) .'.suspend_sale') ?></div>
					<?php
					// Only show this part if the payment covers the total
					if(!$pos_mode && isset($customer))
					{
					?>
						<div class='btn btn-sm btn-success' id='finish_invoice_quote_button'><i class="bi-check-circle icon-spacing"></i><?= esc($mode_label) ?></div>
					<?php
					}
					?>

					<div class='btn btn-sm btn-danger pull-right' id='cancel_sale_button'><i class="bi-x-circle icon-spacing"></i><?= lang(ucfirst($controller_name) .'.cancel_sale') ?></div>
				</div>
			<?= form_close() ?>
